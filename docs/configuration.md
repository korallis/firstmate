...
FM_CLASSIFY_PAUSED_VERB=paused     # leading verb for the declared external-wait lifecycle; excluded from FM_CAPTAIN_RE and distinct from blocked (docs/architecture.md owns lifecycle and precedence)
...
FM_PAUSE_RESURFACE_SECS=3600       # seconds after first observation before a current idle declared pause re-surfaces for a recheck in the watcher or away-mode daemon
