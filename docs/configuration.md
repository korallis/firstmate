...
FM_BACKEND_HERDR_BARE_PROMPT_RE='^(❯|›)'  # herdr-only: verified agent glyphs recognized as an UNBORDERED (bare) composer row, e.g. claude's ❯ or codex's ›, written as an alternation so a C-locale byte-wise grep cannot false-match box-drawing border bytes; shell glyphs remain unknown rather than empty, and de-emphasised ghost/placeholder text (dim or dark-truecolor) after an agent prompt reads empty via the shared fm_composer_strip_ghost (docs/herdr-backend.md "Incident (2026-07-08)", "Incident (2026-07-10)")
...
FM_TURNEND_ENSURE_LOG_MAX_BYTES=262144   # size cap for the grok turn-end guard's watcher-ensure log (state/.turnend-watch-ensure.log; docs/turnend-guard.md)
