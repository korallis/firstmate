...
FM_PR_READY_SCAN_INTERVAL=30   # minimum seconds between branch-matched PR-ready rechecks per recorded no-mistakes ship task; at most one eligible task is scanned per watcher loop
...
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the current-state reader used by working/paused watcher triage and PR-ready scanning
