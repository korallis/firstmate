...
`bin/fm-watch.sh` splices this in as the watcher's terminal wait (`event_wait_or_sleep`, replacing the blind `sleep POLL` for push-capable homes): on a returned `blocked` it maps `pane_id -> <session>:<pane_id> -> task`, exempts `kind=secondmate` endpoints, and enqueues an immediate `stale` wake.
A fresh native `blocked` edge remains actionable even when the status log contains an older open pause; the current-state reconciliation decides pause precedence on the polling path.
...
- **RESOLVED: a "paused / awaiting-external" crew state for the stale-wedge escalation.** Raised alongside the 2026-07-07 incident: an in-flight crew intentionally idling on a known external wait (a vendor rate limit, say) still tripped `bin/fm-supervise-daemon.sh`'s "stale persisted ... (possible wedge)" escalation exactly like a genuinely wedged crew, with no way to mark the wait as expected.
  Fixed by the `paused:` external-wait lifecycle: a crew declares a deliberate wait, and both `bin/fm-watch.sh` and `bin/fm-supervise-daemon.sh` absorb it only when the shared current-state reconciliation confirms the pause, re-surfacing it for a recheck on a long cadence instead of a wedge escalation.
  See [architecture.md](architecture.md#event-driven-supervision) for keyed resolution and run-state precedence, `AGENTS.md` section 8 for the operating rule, and the crew-facing brief contract in `bin/fm-brief.sh`.
