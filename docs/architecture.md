...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, check-script output such as PR merge polling or an X-mode mention, authoritative branch-matched PR-ready transitions, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
On its own persisted cadence, the watcher re-reads `bin/fm-crew-state.sh` for every recorded no-mistakes ship task, including busy and changing panes, so a background CI fixer reaching checks-passed cannot remain hidden behind an unchanged pane, stale status event, or ended agent turn.
`bin/fm-pr-ready-lib.sh` owns the exact readiness, deduplication, and supersession contract; its `check:` wake only routes Firstmate to the existing recorded PR review/merge path and performs no approval or external action.
...
During no-mistakes' `ci` monitor phase, it also reads the ci step log because `axi status` reports both "still waiting on checks" and "checks green, waiting on merge" as `ci,running`.
The most recent recognized ci log marker determines current state, while its stable generation refines PR-ready identity without making newly readable history look like a newly observed relapse.
Readiness supersession is instead persisted when the bounded watcher scan authoritatively observes a non-ready state, as owned by `bin/fm-pr-ready-lib.sh`.
