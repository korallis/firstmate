...
state/               volatile runtime signals; gitignored
  ...
  .hash-* .count-* .stale-* .stale-since-* .paused-* .wedge-escalations-* .seen-* .hb-surfaced-* .pr-ready-* .last-* .heartbeat-streak   watcher internals; never touch
...
During absorbed validation it also emits one deduplicated `check:` wake when authoritative branch-matched state reaches PR-ready without a new crew event; handle that through the normal PR-ready approval and recorded merge path.
...
4. `check:` a per-task poll or authoritative background PR-ready transition fired (usually a merge, checks-green run, or X mode when enabled); act on it.
